...
async function submitUserMessage(userInput: string, model: string): Promise<UIState>{
  "use server";
  ...
  (async () => {
    try {
      const response = await experimental_streamText({
        ...
        tools: {
          search_news_articles: {
            description: "Search for relevant news articles based on the user's query.",
            parameters: z.object({
              query: z.string().describe("The search query provided by the user."),
            }),
          },
          generate_report_summary: {
            description: "Summarize up to 4 reports and display the results intuitively.",
            parameters: z.object({
              reportIds: z.array(z.string()).describe("The IDs of the reports to summarize."),
            }),
          },
        },
        system: `\
      ...
      for await (const delta of response.fullStream) {
        ...
        if (toolName === 'search_news_articles') {
          const {query} = args
          ...
          aiState.done({
            ...
            messages: [
              ...aiState.get().messages,
              {
                id: nanoid(),
                role: 'assistant',
                content: `Here are some news articles about ${query}`,
                display: {
        ...
        } else if (toolName === 'generate_report_summary') {
