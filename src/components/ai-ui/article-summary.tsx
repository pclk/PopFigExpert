...
export default function ArticleSummary({ articles, args }: ReportSummaryProps) {
  ...
  const suggestions = articles.reduce(
    (acc, article, index) => {
      acc[`Summarize ${article.title.substring(0, 10)}..`] =
        `Could you summarize the article titled "${article.title}", in your own words, without using the generate articles function? Do note that the function has already ran, and you can proceed summarizing as the next step.`;
      return acc;
    },
